...
- **Realizzazione Frontend Web Single-Player Streamlit (`src/scummbar_chat/streamlit/`)**:
  ...
  - Perfezionata la funzione `format_streamlit_narrative()`: sia la narrazione ambientale (`_testo_`) che le azioni dei personaggi (`*azione*`) forzano la font-family monospaziata via `font-family: monospace !important;` e tag `<code>`, mentre il testo parlato del bot rimane nel font normale sans-serif per la massima leggibilità.
